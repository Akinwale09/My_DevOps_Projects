...
2. Pull the Gatus Docker image:
    ...
    ![DockerPull](./img/02.%20DockerPull.png)

3. Create a directory for Gatus configuration:

    `mkdir gatus && cd gatus`

    ![gatusDir](./img/03.%20GatusDir.png)
...
4. Start Gatus using Docker with a basic setup:
    ...
    ![StartGatusUsingDocker](./img/05.%20StartGatus.png)

    ![DockerStarted](./img/04.%20DockerStarted.png)

5. Access the Gatus dashboard in your browser at `http://localhost:8080`

    ![GatusDashboard](./img/06.%20GatusDashboard.png)
...
1. Inside the `gatus/config` directory, create a `config.yaml` file.
    ...
    ![configfile](./img/07.%20configfile.png)

2. Add a simple configuration to monitor a website:

    ![configFile](./img/08.%20configfile.png)

    ![sampleconfigfile](./img/09.%20sampleconfig.png)

3. Restart the Gatus container to apply the configuration:

    `docker restart gatus`

    ![GatusRestart](./img/10.%20GatusRestart.png)

    ![ExampleEndPoint](./img/11.%20ExampleEndPoint.png)
...
1. Add another endpoint to the config.yaml file for monitoring:
    ...
    ![AnotherEndPoint](./img/12.%20AnotherEndPoint.png)

    ![ConfigFile](./img/13.%20GitHubEndPoint.png)

2. Restart Gatus and verify the new endpoint appears on the dashboard.

    `docker restart gatus`

    ![GitHubEndPoint](./img/14.%20GatusGitHub.png)

4. Simulate a failure by adding a non-existent endpoint and observe the behavior:

    ![Simulatefailure](./img/15.%20SimulateFailure.png)

    ![Failure](./img/16.%20Failure.png)
...
2. Add an alert configuration to config.yaml:
    ...
    ![AlertConfig](./img/18.%20Alert.png)
...
1. Access the Gatus dashboard to view uptime statistics for each endpoint.
    ...
    ![DashBoard](./img/19.%20DashBoard.png)
